...
app.get("/states/", async (request, response) => {
  ...
  response.send(
    statelist.map((obj) => {
      convertstateDbObjectToResponseObject(obj);
    })
  );
});
...
app.get("/states/:stateId/", async (request, response) => {
  ...
  const stateListQuery = `SELECT * 
  FROM state 
  WHERE student_id='${stateId}';`;
...
app.get("/districts/:districtId/", async (request, response) => {
  ...
  const districtListQuery = `SELECT * 
  FROM district 
  WHERE district_id='${districtId}';`;
  const district = await db.get(stateListQuery);
